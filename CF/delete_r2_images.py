#!/usr/bin/env python3
"""Delete Motorgy image objects already stored in Cloudflare R2.

Targets keys under the site prefix that look like scrape image uploads:
  motorgy/year=…/month=…/day=…[/part=…]/images/{ad_id}/01.webp

Excel, JSON, and monitor artifacts are never deleted.
...
def is_image_object(key: str) -> bool:
    ...
    if "/images/" not in normalized:
        return False
    # Never touch monitor / spreadsheet / json artifacts even if misnamed.
    protected_markers = (
        "/excel_files/",
        "/json-files/",
        "/json version/",
...
def list_image_objects(
    ...
    print(f"Listing objects under r2://{bucket}/{prefix}")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Delete Motorgy scrape images from Cloudflare R2"
    )
    parser.add_argument(
        "--site-prefix",
        default=os.getenv("SITE_PREFIX", DEFAULT_SITE_PREFIX),
        help=f"Site R2 prefix (default: {DEFAULT_SITE_PREFIX})",
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print("=" * 60)
    print(f"Motorgy R2 image cleanup — {mode}")
